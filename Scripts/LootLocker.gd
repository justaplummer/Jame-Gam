...
@onready var l_edit_player_name: LineEdit = $LEdit_player_name
...
func _upload_score(score: float, name : String):
	var data = { "score": str(score*100), "member_id" : member_id, "metadata" : name}
	var headers = ["Content-Type: application/json", "x-session-token:"+session_token]
...
func _on_button_pressed() -> void:
	_upload_score(stopwatch.get_time_as_number(), l_edit_player_name.text )
